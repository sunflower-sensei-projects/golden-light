...
// Set up Party sprites and animations
function scr_make_char_sprites(_idle, _walk, _run, _climb, _push, _slip, _fall, _land_soft, _land_hard, _jump) {
	return {
		...
		slip      : _slip,
		fall      : _fall,
		land_soft : _land_soft,
		land_hard : _land_hard,
		jump      : _jump
	};
}
...
if _check == 0
{
	show_debug_message("Successfully loaded character data!");
}
else
{
	show_debug_message("Failed to load character data!");
}

_Party = [];
addProtagToParty("Joshua");